...
import { deleteExamAction, saveExamAction, toggleExamAction } from "./actions";
...
type ConfirmState = { kind: "toggle" | "delete"; item: Exam };

export function ExamManager({ items, filters }: { items: Exam[]; filters?: React.ReactNode }) {
  const [opened, modal] = useDisclosure(false); const [pending, startTransition] = useTransition(); const [confirm, setConfirm] = useState<ConfirmState | null>(null);
  ...
  const runConfirm = () => confirm && startTransition(async () => { const result = confirm.kind === "delete" ? await deleteExamAction(confirm.item.id) : await toggleExamAction(confirm.item.id, !confirm.item.isActive); notifications.show({ color: result.ok ? "green" : "red", message: result.message }); if (result.ok) setConfirm(null); });
  const confirmTitle = confirm?.kind === "delete" ? "Delete exam?" : confirm?.item.isActive ? "Deactivate exam?" : "Activate exam?";
  const confirmMessage = confirm?.kind === "delete" ? "This permanently deletes the exam, its subjects, topics, questions, and related study history." : confirm?.item.isActive ? "Learners will no longer be able to start sessions for this exam." : "This exam will become available to learners.";
  const confirmLabel = confirm?.kind === "delete" ? "Delete exam" : confirm?.item.isActive ? "Deactivate" : "Activate";
  const confirmColor = confirm?.kind === "delete" ? "red" : confirm?.item.isActive ? "orange" : "green";
  return <><PageHeader title="Exams" actions={<Button leftSection={<IconPlus size={16} />} onClick={openCreate}>New exam</Button>} />{filters && <div className="page-filters">{filters}</div>}{items.length === 0 ? <EmptyState title="No exams found" description="Create the first exam to start organizing subjects and questions." /> : <div className="data-table-wrap"><Table highlightOnHover verticalSpacing="sm"><Table.Thead><Table.Tr><Table.Th>Exam</Table.Th><Table.Th>Marks</Table.Th><Table.Th>Target</Table.Th><Table.Th>Duration</Table.Th><Table.Th>Status</Table.Th><Table.Th w={48}><span className="sr-only">Actions</span></Table.Th></Table.Tr></Table.Thead><Table.Tbody>{items.map((item) => <Table.Tr key={item.id}><Table.Td><Text fw={600}>{item.name}</Text><Text fz="xs" c="dimmed">{item.code}</Text></Table.Td><Table.Td>{item.totalMarks}</Table.Td><Table.Td>{item.targetScore}</Table.Td><Table.Td>{item.durationMinutes} min</Table.Td><Table.Td><StatusBadge active={item.isActive} /></Table.Td><Table.Td><Menu position="bottom-end"><Menu.Target><ActionIcon variant="subtle" aria-label={`Actions for ${item.name}`}><IconDots size={18} /></ActionIcon></Menu.Target><Menu.Dropdown><Menu.Item leftSection={<IconEdit size={16} />} onClick={() => openEdit(item)}>Edit</Menu.Item><Menu.Item leftSection={<IconPower size={16} />} color={item.isActive ? "orange" : "green"} onClick={() => setConfirm({ kind: "toggle", item })}>{item.isActive ? "Deactivate" : "Activate"}</Menu.Item><Menu.Item leftSection={<IconTrash size={16} />} color="red" onClick={() => setConfirm({ kind: "delete", item })}>Delete</Menu.Item></Menu.Dropdown></Menu></Table.Td></Table.Tr>)}</Table.Tbody></Table></div>}
