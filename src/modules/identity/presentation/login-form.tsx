...
export function LoginForm({ callbackUrl }: { callbackUrl?: string }) {
  ...
  return <form onSubmit={submit} noValidate><Stack>{error && <Alert color="red" icon={<IconAlertCircle size={18} />}>{error}</Alert>}<TextInput label="Email" placeholder="you@example.com" autoComplete="email" required leftSection={<IconAt size={16} />} {...form.getInputProps("email")} /><PasswordInput label="Password" placeholder="Your password" autoComplete="current-password" required leftSection={<IconLock size={16} />} {...form.getInputProps("password")} /><Button type="submit" fullWidth loading={pending}>Sign in</Button></Stack></form>;
}
