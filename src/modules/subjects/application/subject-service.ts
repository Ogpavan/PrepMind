...
export const subjectService = {
  ...
  async remove(id: string) {
    ...
    if (!parsed.success) throw new ApplicationError("VALIDATION", "Select a valid subject to delete.");
    const summary = await examRepository.removeSubjectCascade(parsed.data);
    if (!summary) throw new ApplicationError("NOT_FOUND", "Subject not found.");
    return summary;
  },
};
