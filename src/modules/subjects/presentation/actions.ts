...
export async function deleteSubjectAction(id: string): Promise<ActionResult> { try { await assertRole(["SUPER_ADMIN", "ADMIN"]); const summary = await subjectService.remove(id); revalidatePath("/admin/subjects"); revalidatePath("/admin/topics"); revalidatePath("/admin/questions"); revalidatePath("/admin/dashboard"); return { ok: true, data: undefined, message: `Subject deleted with ${summary.topics} topic${summary.topics === 1 ? "" : "s"} and ${summary.questions} question${summary.questions === 1 ? "" : "s"}.` }; } catch (error) { return toActionError(error); } }
