...
import { DotsThree as IconDots, PencilSimple as IconEdit, Plus as IconPlus, Power as IconPower, Trash as IconTrash } from "@phosphor-icons/react/ssr";
...
import { deleteSubjectAction, saveSubjectAction, toggleSubjectAction } from "./actions";
...
export function SubjectManager({ items, exams, filters }: { items: SubjectListItem[]; exams: Exam[]; filters?: React.ReactNode }) {
  ...
  const [opened, modal] = useDisclosure(false); const [pending, startTransition] = useTransition(); const [confirm, setConfirm] = useState<{ kind: "toggle" | "delete"; item: SubjectListItem } | null>(null); const form = useForm({ initialValues: initial, validate: zod4Resolver(subjectSchema) });
  ...
  const runConfirm = () => confirm && startTransition(async () => { const result = confirm.kind === "delete" ? await deleteSubjectAction(confirm.item.id) : await toggleSubjectAction(confirm.item.id, !confirm.item.isActive); notifications.show({ color: result.ok ? "green" : "red", message: result.message }); if (result.ok) setConfirm(null); });
  const confirmTitle = confirm?.kind === "delete" ? "Delete subject?" : confirm?.item.isActive ? "Deactivate subject?" : "Activate subject?";
  const confirmMessage = confirm?.kind === "delete" ? "This permanently deletes the subject, its topics, questions, and related study history." : confirm?.item.isActive ? "This subject will not be available for new study sessions." : "This subject will become available to learners.";
  const confirmLabel = confirm?.kind === "delete" ? "Delete subject" : confirm?.item.isActive ? "Deactivate" : "Activate";
  const confirmColor = confirm?.kind === "delete" ? "red" : confirm?.item.isActive ? "orange" : "green";
  return <><PageHeader title="Subjects" actions={<Button leftSection={<IconPlus size={16} />} onClick={openCreate} disabled={exams.length === 0}>New subject</Button>} />{filters && <div className="page-filters">{filters}</div>}{items.length === 0 ? <EmptyState title="No subjects found" description="Add subjects under an exam to organize topics and questions." /> : <div className="data-table-wrap"><Table highlightOnHover verticalSpacing="sm"><Table.Thead><Table.Tr><Table.Th>Subject</Table.Th><Table.Th>Exam</Table.Th><Table.Th>Order</Table.Th><Table.Th>Status</Table.Th><Table.Th w={48}>Actions</Table.Th></Table.Tr></Table.Thead><Table.Tbody>{items.map((item) => <Table.Tr key={item.id}><Table.Td><Text fw={600}>{item.name}</Text><Text fz="xs" c="dimmed">{item.code}</Text></Table.Td><Table.Td>{item.examName}</Table.Td><Table.Td>{item.displayOrder}</Table.Td><Table.Td><StatusBadge active={item.isActive} /></Table.Td><Table.Td><Menu position="bottom-end"><Menu.Target><ActionIcon variant="subtle" aria-label={`Actions for ${item.name}`}><IconDots size={18} /></ActionIcon></Menu.Target><Menu.Dropdown><Menu.Item leftSection={<IconEdit size={16} />} onClick={() => openEdit(item)}>Edit</Menu.Item><Menu.Item leftSection={<IconPower size={16} />} color={item.isActive ? "orange" : "green"} onClick={() => setConfirm({ kind: "toggle", item })}>{item.isActive ? "Deactivate" : "Activate"}</Menu.Item><Menu.Item leftSection={<IconTrash size={16} />} color="red" onClick={() => setConfirm({ kind: "delete", item })}>Delete</Menu.Item></Menu.Dropdown></Menu></Table.Td></Table.Tr>)}</Table.Tbody></Table></div>}
  ...
  <ConfirmDialog opened={Boolean(confirm)} onClose={() => setConfirm(null)} onConfirm={runConfirm} title={confirmTitle} message={confirmMessage} confirmLabel={confirmLabel} color={confirmColor} loading={pending} /></>;
}
